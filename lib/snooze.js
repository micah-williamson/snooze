(function() {
				...
				for(var i = 0; i < modules.length; i++) {
					...
					if(!this.moduleExists(submod)) {
						...
						if(path) {
							...
							throw new Error('Unable to find submodule in node_modules (recursively): ' + submodule);
						}
					}
				}
			}
	...
	Snooze.prototype.findSubmodPath = function(submod) {
		var packagePaths = this.glob('**/node_modules/' + submod + '/package.json', {cwd: process.cwd()});
		var packagePath = packagePaths[0];

		if(packagePath) {
			var packageJSON = JSON.parse(fs.readFileSync(packagePath));
			var mainPaths = this.glob('**/node_modules/' + submod + '/' + packageJSON.main, {cwd: process.cwd()});
