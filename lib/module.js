(function() {
		...
		for(i = 0; i < this.modules.length; i++) {
			...
			for(k = 0; k < processes.length; k++) {
				...
				for(var j = 0; j < configPreprocessors.length; j++) {
					if((configPreprocessors[j]+'').replace(/\s/g, '') === (process+'').replace(/\s/g, '')) {
						duplicate = true;
						break;
					}
				}
	...
	Module.prototype.importModuleImportProcesses = function() {
		...
		for(i = 0; i < this.modules.length; i++) {
			...
			for(k = 0; k < processes.length; k++) {
				var process = processes[k];
				var duplicate = false;
				for(j = 0; j < importProcesses.length; j++) {
					if((importProcesses[j]+'').replace(/\s/g, '') === (process+'').replace(/\s/g, '')) {
						duplicate = true;
						break;
					}
				}
	...
	Module.prototype.importModules = function() {
		...
		for(i = 0; i < this.modules.length; i++) {
			...
			if(!this.importedModules[modName]) {
				...
				this.log(('Importing ' + _mod.getName() + ' into ' + this.getName()).blue);
	...
	Module.prototype.registerEntityGroupsFromPath = function(patterns) {
		...
		var files = this.snooze.glob(patterns, {cwd: path});
	...
	Module.prototype.registerEntitiesFromPath = function(patterns, path) {
		if(!path) {
			path = process.cwd() + '/';
	...
	Module.prototype.registerImportProcessesFromPath = function(patterns, path) {
		if(!path) {
			path = process.cwd() + '/';
	...
	Module.prototype.registerConfigPreprocessorsFromPath = function(patterns, path) {
		if(!path) {
			path = process.cwd() + '/';
